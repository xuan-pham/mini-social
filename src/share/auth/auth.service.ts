...
export class AuthService {
    ...
    private async verifyPassword(plainTextPassword: string, hashedPassword: string) {
        const isPasswordMatching = await bcrypt.compare(plainTextPassword,
            hashedPassword
        );
    ...
    private async hashPassword(pass: string) {
        const match = await bcrypt.hash(pass, 10);
        return match;
    }
    ...
    async checkEmailActive(email: string) {
        const user = await this.userService.getUserByEmail(email);
        if (!user.isStatus) throw new HttpException('Account is not active,Please check your email again', HttpStatus.BAD_REQUEST);
